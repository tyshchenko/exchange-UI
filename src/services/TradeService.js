...
class TradeService {
  ...
  async getLedger() {
    ...
    if (data.Expired==1) {
      ...
      arr.push({'wallet_type':'exchange','currency':'BTC','locked_bal':Number(data.result.BTC.freeze).toFixed(6),'avail_bal':Number(data.result.BTC.available).toFixed(6),'total_bal':(Number(data.result.BTC.freeze) + Number(Number(data.result.BTC.available))).toFixed(6),});
